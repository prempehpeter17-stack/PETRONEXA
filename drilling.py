"""PetroNexa Streamlit drilling engineering UI."""
import os
...
def render_drilling_sidebar():
    """Render drilling-specific sidebar controls and return their values."""
    with st.sidebar:
        if os.path.exists("logo.png"):
            st.image("logo.png", width=34)
        st.markdown("### Drilling Engineering Controls")
        ...
        if uploaded_file is not None and not st.session_state.parsed:
            try:
                ...
                st.sidebar.success(f"Parsed · PV={data['pv_cp']} · YP={st.session_state.auto_yp} · MW={data['mw_ppg']} ppg")
        ...
        grad_df = st.data_editor(st.session_state.gradient_df, num_rows="dynamic", key="gradient_editor")
    ...
    return {
        ...
        "tvd": tvd,
        "flow_rate": flow_rate,
        "surface_mw": surface_mw,
        "pv": pv,
        "yp": yp,
    }


def render_drilling_workspace(controls, user_data):
    """Render all existing drilling engineering tools without changing their calculations."""
    total_depth = controls["total_depth"]
    tvd = controls["tvd"]
    flow_rate = controls["flow_rate"]
    surface_mw = controls["surface_mw"]
    pv = controls["pv"]
    yp = controls["yp"]
    project_name = controls["project_name"]
    rig_name = controls["rig_name"]
